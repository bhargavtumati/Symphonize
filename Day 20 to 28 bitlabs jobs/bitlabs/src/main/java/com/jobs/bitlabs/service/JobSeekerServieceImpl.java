...
public class JobSeekerServieceImpl implements JobSeekerService {
	...
	@Override
	public String applyForJob(Long userId, String jobId) {
	   ...
	   JobSeekerDto userDto = this.UserToDto(user);
	    ...
	    if (isUserQualifiedForJob(userDto, jobDto)) {
	    	user.getAppliedJobStatus().put(job,JobStatus.Screening);
 
	        // Add the user to the job's list of applicants
	        job.getApplicantStatus().put(user,JobStatus.Screening);
	    ...
	    return "You dont have enough eligibity to apply"; 
	}
	...
	private boolean matchesUserProfile(JobSeeker user, CompanyJob job) {
     ...
     public JobStatus viewJobStatus(String companyjobid, Long id) {
