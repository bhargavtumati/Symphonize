...
async def get_recruiter_by_id(
    ...
    try:
        # Fetch the recruiter using the get_by_id method
        recruiter = Recruiter.get_by_id(session=session, id=recruiter_id)
        
        # If recruiter not found, raise an HTTPException
        if recruiter is None:
            raise HTTPException(status_code=404, detail="Recruiter not found")
        
        # Return the recruiter data in a response
        return {
