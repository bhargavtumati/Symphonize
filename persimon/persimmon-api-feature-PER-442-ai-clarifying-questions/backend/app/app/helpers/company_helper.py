...
def get_or_create_company(job: JobModel, session: Session, created_by: str):
    ...
    return company_record, company_code
...
def handle_company_association(
    ...
    if (job_exists.is_posted_for_client and job.is_posted_for_client) or (
        ...
        company_code = company_data["name"].capitalize()[:3]
        ...
        if company_exists:
            ...
            company_code = company_exists.name.capitalize()[:3]
    
    job_code = jobh.generate_job_code(session=session, company_code=company_code)
    job_exists.code = job_code
