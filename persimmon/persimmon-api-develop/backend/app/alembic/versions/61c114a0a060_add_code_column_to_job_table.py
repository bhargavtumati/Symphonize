...
def populate_data():
    ...
    jobs = connection.execute(
        sa.text(
            """
            SELECT job.id, job.company_id, job.code, job.meta, company.name AS company_name
            FROM job
    ...
    for job in jobs:
        job_id = job.id
        email_id = job.meta['audit']['created_by']['email']
        domain = regexh.get_domain_from_email(email_id)
        company_row = connection.execute(sa.text("SELECT name FROM company WHERE domain = :domain"), {'domain': domain} ).fetchone()
        company_code = company_row.name.capitalize()[:3]
