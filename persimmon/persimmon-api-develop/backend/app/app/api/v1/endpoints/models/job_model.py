...
from app.utils.validators import validate_letters_and_numbers, is_non_empty, validate_length, validate_numeric_range, has_proper_characters, validate_job_location, validate_decimal_point
...
class JobModel(BaseModel):
    ...
    def validate_min_salary(cls, min_salary):
        validate_decimal_point(value=min_salary)
        return validate_numeric_range(value=min_salary, min_val=1, max_val=99, field_name=MINIMUM_SALARY_FIELD)

    @field_validator('max_salary')
    def validate_max_salary(cls, max_salary):
        validate_decimal_point(value=max_salary)
        return validate_numeric_range(value=max_salary, min_val=2, max_val=100, field_name=MAXIMUM_SALARY_FIELD)     
