...
async def create_or_update_customization(
    name: str,
    enable_cover_photo: bool = Form(True),
    heading: str = Form("Join Us"),
    ...
    enable_dark_mode: bool = Form(False),
    ...
    try:
        ...
        if not isinstance(primary_colors_list, list):
            raise ValueError("primary_colors must be a list")
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid primary_colors format")
