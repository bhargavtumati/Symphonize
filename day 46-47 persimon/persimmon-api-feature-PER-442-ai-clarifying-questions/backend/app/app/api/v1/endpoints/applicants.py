...
async def get_applicant_resume(applicant_id: int, db: Session = Depends(get_db), token: dict = Depends(verify_firebase_token)):
    """
    Retrieve an applicant's resume.
    ...
    Returns:
        dict: The applicant's resume details.
    """
    ...
    # Now using the correct key name 'original_resume'
    original_resume = applicant_details.get('orignal_resume')
    ...
    # Sanitize the original resume to remove any invalid characters for Windows file system
    sanitized_resume = re.sub(r'[<>:"/\\|?*]', '_', original_resume)  # Replace invalid chars with '_'

    # Define the local path for storing the resume dynamically using the sanitized filename
    local_file_path = os.path.join(r"C:\Users\SESPL\Downloads", f"{sanitized_resume}.pdf")
    ...
    try:
        # Await the download function to get the resume and capture its response
        download_response = await s3h.download_from_gcp(bucket_name, original_resume, local_file_path)
        
        # If download is successful, return the success message and response
        if "Failed" in download_response:
            raise HTTPException(status_code=500, detail=f"Error downloading the file: {download_response}")
        
        return {"message": "Resume successfully downloaded", "download_response": download_response, "resume_path": local_file_path}
    
    except PermissionError:
        # Return an error if permission is denied
        raise HTTPException(status_code=500, detail="Error downloading the file: Permission denied")
    except Exception as e:
