...
async def download_from_gcp(bucket_name: str, destination_path: str, local_file_path: str) -> str:
    # Remove the gs:// prefix if it's present in the path
    destination_path = destination_path.replace('gs://persimmon-ai/', '')

    # Setup credentials and storage client
    credentials = service_account.Credentials.from_service_account_info(SA_KEY)
    storage_client = storage.Client(credentials=credentials, project=SA_KEY["project_id"])
    ...
    try:
        # Download the file from GCP to the local system
        blob.download_to_filename(local_file_path)
        print(f"File successfully downloaded to {local_file_path}.")
        return local_file_path
    except Exception as e:
        print(f"Error downloading the file: {e}")
        return f"Failed to download file from {destination_path}."
