...
async def create_recruiter_endpoint(
    ...
    try:
        existing_recruiter = Recruiter.get_by_whatsapp_number(
            session=session, whatsapp_number=recruiter.whatsapp_number
        )
        ...
        if str(e) == "WhatsAppNumberExists":
            raise HTTPException(
                status_code=409,
                detail="This WhatsApp number is already registered. Please use a different number",
            )
        ...
        if "duplicate key value violates unique constraint" in str(e.orig):
            raise HTTPException(
                status_code=409,
                detail="Email already exists, please provide a new Email ID",
            )
        else:
