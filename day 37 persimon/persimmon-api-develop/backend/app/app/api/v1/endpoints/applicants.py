...
def get_applicants(
   ...
   # token: dict = Depends(verify_firebase_token),
    ...
    try:
        page_size = 20
        total_count = Applicant.get_count(session=session, job_id=job_id, stage_uuid=stage_uuid, name=name)
        pagination = get_pagination(page=page,page_size=page_size,total_records=total_count)
        posted_applicants = Applicant.get_all(session=session, limit=page_size, offset=pagination['offset'], job_id=job_id, stage_uuid=stage_uuid, name=name)
        return {
     ...
     #   updated_by = token['email']
        ...
        if len(applicant_uuids) > 0:
            ...
            for applicant_uuid in applicant_uuids:
                existing_applicant: Applicant = Applicant.get_by_uuid(session=session, uuid=applicant_uuid)
