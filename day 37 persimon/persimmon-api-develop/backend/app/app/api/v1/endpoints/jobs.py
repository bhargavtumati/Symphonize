...
def create_job(
    ...
    try:
        ...
        if company_code:
            sequence_number = Job.get_next_sequence_number(
                session=session, company_code=company_code
            )
...
def get_jobs(
    ...
    try:
        ...
        pagination = mathh.get_pagination(
    ...
    try:
        ...
        if job.is_posted_for_client:
            ...
            domain = get_domain_from_email(
                job_exists.meta["audit"]["created_by"]["email"]
            )
        ...
        if (job_exists.is_posted_for_client and job.is_posted_for_client) or (
            ...
            if company_exists:
                ...
                company_exists.meta.update(
                    dbh.update_meta(company_exists.meta, updated_by)
                )
...
def get_job_by_id(
    ...
    try:
        ...
        company_data = Company.get_by_id(
            session=session, company_id=job_data.company_id
        )
        if not company_data:
            raise HTTPException(status_code=404, detail="Company or client not Found")

        return {"job": job_data, "company": company_data, "status": status.HTTP_200_OK}
