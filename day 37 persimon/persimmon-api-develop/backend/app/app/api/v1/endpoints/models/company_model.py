...
class CompanyModel(BaseModel):
    ...
    def validate_name(cls, name):
        ...
        validate_length(
            value=name, min_len=3, max_len=50, field_name=COMPANY_NAME_FIELD
        )
        if not re.match(r"^[a-zA-Z0-9&\-\.\'\s]+$", name):
            raise ValueError(
                "Company name can only contain alphanumeric characters, spaces, &, -, ., and '"
            )
        return name
    ...
    def validate_website(cls, website):
        ...
        validate_length(
            value=website, min_len=5, max_len=100, field_name=COMPANY_WEBSITE_FIELD
        )
        return website
    ...
    def validate_linkedin(cls, linkedin):
        ...
        validate_length(
            value=linkedin, min_len=5, max_len=100, field_name=COMPANY_LINKEDIN_FIELD
        )
        if not re.match(
            r"^https://(www\.)?linkedin\.com/company/[a-zA-Z0-9\-_]{3,}(/.*)?/?$",
            linkedin,
        ):
