    ...
    try:
        ...
        if len(removed_stage_uuids) > 0:
                        ...
                        updates.append(
                            {
                                "id": document["id"],
                                "stage_uuid": {
                                    "set": [existing_stages.stages[0]["uuid"]]
                                },
                            }
                        )
            ...
            for removed_stage_uuid in removed_stage_uuids:
                existing_applicants: list[Applicant] = Applicant.get_by_stage_uuid(
                    session=session, stage_uuid=removed_stage_uuid
                )
        ...
        return {"message": "Stages updated successfully", "status": status.HTTP_200_OK}
