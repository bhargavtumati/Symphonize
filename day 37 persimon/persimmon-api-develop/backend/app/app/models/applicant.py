...
class Applicant(Base):
    ...
    @classmethod
    def get_count(
        cls, session: Session, job_id: int, stage_uuid: str, name: str = None
    ) -> int:
        ...
        if name:
            query = query.filter(
                cls.details["personal_information"]
                .cast(JSONB)["full_name"]
                .astext.ilike(f"%{name}%")
            )
        return query.count()
    ...
    def get_all(
        ...
        return (
            query.order_by(desc(cls.meta["audit"]["created_at"]))
            .offset(offset)
            .limit(limit)
            .all()
        )
