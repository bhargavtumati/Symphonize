...
class Company(Base):
    ...
    type: Mapped[CompanyTypeEnum] = mapped_column(
        Enum(CompanyTypeEnum, name="CompanyType", schema="enum"), nullable=False
    )
    ...
    def get_by_name(cls, session: Session, company_name: str):
        if company_name:
            return (
                session.query(cls).filter(cls.name.ilike(f"%{company_name}%")).first()
            )
