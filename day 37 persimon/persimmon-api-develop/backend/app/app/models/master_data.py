...
class MasterData(Base):
    ...
    def seed_master_data(cls, session: Session):
        ...
        industry_types_json_file = os.path.join(
            current_dir, "..", "datasets", "industry_types.json"
        )
        indian_cities_json_file = os.path.join(
            current_dir, "..", "datasets", "indian_cities.json"
        )
        ...
        names = {
            entry.value["name"]
            for entry in industry_type_exists
            if "name" in entry.value
        }
        location_exists = session.query(cls).filter_by(type="location").all()
        states = {
            entry.value["state"] for entry in location_exists if "state" in entry.value
        }
