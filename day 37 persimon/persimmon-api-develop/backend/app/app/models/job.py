...
class Job(Base):
    ...
    type: Mapped[JobTypeEnum] = mapped_column(
        Enum(JobTypeEnum, name="JobType", schema="enum"), nullable=False
    )
    status: Mapped[JobStatusTypeEnum] = mapped_column(
        Enum(JobStatusTypeEnum, name="JobStatusType", schema="enum"), default="ACTIVE"
    )
    workplace_type: Mapped[WorkplaceTypeEnum] = mapped_column(
        Enum(WorkplaceTypeEnum, name="WorkplaceType", schema="enum"), nullable=False
    )
    ...
    def filter_query(
        ...
        if client_name and len(client_name) and (not company):
            return {"query": query, "allow_retrieve": False}

        query = query.filter(
            func.jsonb_extract_path_text(cls.meta, "audit", "created_by", "email")
            == created_by_email
        )
        if id:
        ...
        return {"query": query, "allow_retrieve": True}
    ...
    def get_next_sequence_number(cls, session: Session, company_code: str):
        max_sequence = (
            ...
            .filter(cls.code.like(f"{company_code}%"))
            .scalar()
        )
