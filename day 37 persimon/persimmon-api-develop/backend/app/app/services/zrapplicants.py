...
async def classify(candidate_id: str, applicant_id: str, job_id: str, session: Session):
    ...
    existing_applicant = ZrApplicant.get_by_id(
        session=session, applicant_id=applicant_id
    )
    ...
    attachment = await zohoh.get_attachment_by_id(
        candidate_id=candidate_id, attachment_id=attachment_id
    )

    existing_resume = Resume.get_by_id(
        session=session, attachment_id=attachment["attachment"]["id"]
    )
    ...
    if job["job"]["id"] and attachment["attachment"]["id"]:
        ...
        result, probs = classifierh.classify_message(
            message=message,
            model_version=settings.CLASSIFIER_VERSION,
            vectorizer_version=settings.VECTORIZER_VERSION,
        )
        ...
        match = {"result": result, "score": match_score, "probabilities": probabilities}

        # create candidacy
        candidacy_record = Candidacy(
            job_id=persimmon_job_id, resume_id=persimmon_resume_id, match=match
        )
        ...
        await zohoh.update_applicant_by_id(
            applicant_id=applicant_id, applicant_details=json_data
        )
