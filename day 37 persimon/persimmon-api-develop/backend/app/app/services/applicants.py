...
async def process_resume(
    file, session: Session, created_by, job_id, job_code
) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        if file.filename.endswith(".docx"):
            ...
            try:
                processed_file_path = await pdfh.convert_docx_to_pdf(
                    extracted_text, output_pdf_path
                )
                ...
                file_success = False
                error_message = (
                    f"Error converting {original_file_name} to PDF: {str(e)}"
                )
        ...
        if processed_file_path:
            ...
            try:
                tasks.append(
                    s3h.upload_to_gcp(BUCKET_NAME, processed_file, gcs_processed_path)
                )
            except Exception as e:
                print(f"Error preparing processed file upload: {str(e)}")

        # Upload files concurrently
        gcp_paths = {"original_resume": "", "processed_resume": ""}
        try:
        ...
        if file_success:
            try:
                ...
                default_stages = [
                    ...
                    {"uuid": str(uuid.uuid4()), "name": "Deployed"},
                    {"uuid": str(uuid.uuid4()), "name": "Rejected"},
                ]

                if flatten_resume:
                    try:
                        stages_existing: Stages = Stages.get_by_id(
                        ...
                        try:
                            flatten_resume_solr = await jsonh.flatten_resume_data_solr(
                                generated_json
                            )
                            ...
                            raise HTTPException(
                                status_code=400,
                                detail="Invalid request or missing required fields."
                            )

                        if not stages_existing:
                            recruiter = Recruiter.get_by_email_id(
                                session=session, email=created_by
                            )
                            stages = Stages(
                            ...
                            job_created = stages.create(
                                session=session, created_by=created_by
                            )
                ...
                if "Job not found" in str(e):
                    raise HTTPException(
                        status_code=404,
                        detail="Job not found. Please ensure the job ID is correct.",
                    )
                else:
        ...
        if "Job not found" in str(e):
            ...
            raise HTTPException(
                status_code=400, detail="Invalid request or missing required fields."
            )
        else:
...
def construct_query(filters: FilterRequest) -> str:
    ...
    if filters.industry_type:
        for industry in filters.industry_type:
            weight = PREFERENCE_WEIGHTS.get(industry.pref, 0) * (
                industry.max + industry.min
            )
            individual_industry_weights.append(
                f"industry_type : {industry.name}^{weight}"
            )
            overall_industry_weight += weight

            # construct the experience range query
            experience_query_parts.append(
                f"work_experience:[{industry.min} TO {industry.max}]"
            )
    ...
    if filters.remuneration:
        remuneration_query = (
            f"current_ctc:[{filters.remuneration.min} TO {filters.remuneration.max}]"
        )
    ...
    weighted_skills_query = (
        f"(({skills_query})^{overall_skills_weight})" if skills_query else ""
    )
    ...
    if filters.responsibilities:
        ...
        responsibilities_query = " OR ".join(
            [f'responsibilities: "{resp}"' for resp in valid_responsibilities]
        )
        ...
        for pedigree in filters.pedigree:
            if pedigree.specifications:
                for spec in pedigree.specifications:
                    if (
                        spec.spec == "include"
                        and spec.qualification
                        and spec.institution_name
                    ):
    ...
    if (
        ...
        and filters.availability.name
        and filters.availability.value
    ):
    ...
    if filters.soft_skills:
        for soft_skill in filters.soft_skills:
            if (
                soft_skill.name
                and soft_skill.pref
                and soft_skill.min_value
                and soft_skill.max_value
            ):
                ...
                softskills_query_parts.append(
                    f'soft_skills:"{soft_skill.name} - {soft_skill.min_value}"^{min_weight}'
                )
                softskills_query_parts.append(
                    f'soft_skills:"{soft_skill.name} - {soft_skill.max_value}"^{max_weight}'
                )
    ...
    weighted_softskills_query = (
        f"(({softskills_query})^{overall_softskills_weight})"
        if softskills_query
        else ""
    )
    ...
    weighted_transition_behaviour_query = (
        f"(({transition_behaviour_query})^{overall_transition_behaviour_weight})"
        if transition_behaviour_query
        else ""
    )
    ...
    if filters.advanced_filters:
        for advanced_filter in filters.advanced_filters:
            ...
            weight = (
                PREFERENCE_WEIGHTS.get(advanced_filter.preference, 0)
                * advanced_filter.value
            )
            if field_name == "company_size":
                advanced_filter_queries.append(
                    f"company_size:{advanced_filter.value}^{weight}"
                )
            elif field_name == "team_size":
                advanced_filter_queries.append(
                    f"team_size:{advanced_filter.value}^{weight}"
                )
