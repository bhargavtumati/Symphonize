...
async def get_applicant_resume(applicant_id: int, db: Session = Depends(get_db), token: dict = Depends(verify_firebase_token)):
    ...
    # Bucket name where the resumes are stored
    bucket_name = 'persimmon-ai'
    original_resume = original_resume.replace('gs://persimmon-ai/', '')  # Removes gs:// prefix

    try:
        # Assuming s3h.download_from_gcp() returns a URL for the resume
        download_url = await s3h.get_file_url_from_gcp(bucket_name, original_resume)

        if not download_url:
            raise HTTPException(status_code=500, detail="Error generating the download URL")
        
        # Return the URL where the frontend can access the PDF
        return {"message": "Resume successfully fetched", "download_url": download_url}
    
    except Exception as e:
        # Handle any exceptions and return a general error message
...
def delete_applicant(applicant_id: int, db: Session = Depends(get_db)):
    ...
    # If applicant doesn't exist, raise an exception
    if not applicant:
        raise HTTPException(status_code=404, detail="Applicant not found")
    ...
    db.delete(applicant)
    db.commit()
    return {"detail": "Applicant deleted successfully"}