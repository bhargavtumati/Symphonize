...
async def process_resume(file, session: Session, created_by, job_id, job_code) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        # Step 1: Extract text from the resume (async)
        ...
        # Step 2: Convert DOCX to PDF if needed (async)
        ...
        # Step 3: Prepare async upload tasks for both original and processed files
        ...
        tasks = [
            s3h.upload_to_gcp(BUCKET_NAME, file.file, gcs_original_path)  # Original file upload
        ]

        if processed_file_path:
            try:
                tasks.append(s3h.upload_to_gcp(BUCKET_NAME, processed_file_path, gcs_processed_path))  # Processed file upload
            except Exception as e:
                print(f"Error preparing processed file upload: {str(e)}")

        # Step 4: Run the uploads concurrently with asyncio.gather
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        if isinstance(results[0], Exception):
            file_success = False
            error_message = f"File upload failed: {str(results[0])}"
            error_list.append(error_message)

        # If the upload was successful, extract GCP paths
        if file_success:
            gcp_paths = {
                'original_resume': results[0] if isinstance(results[0], str) else '',
                'processed_resume': results[1] if len(results) > 1 and isinstance(results[1], str) else ''
            }
        
        # Step 5: Generate JSON from the extracted text (async)
        if file_success:
            try:
        ...
        # Step 6: Flatten and store JSON data, and upload to Solr
        if file_success:
            try:
                ...
                # Store the flattened resume to the database
                stages_existing: Stages = Stages.get_by_id(session=session, job_id=job_id)
                stage_uuid = stages_existing.stages[0]['uuid'] if len(stages_existing.stages) > 0 else None
                ...
                # Upload flattened resume to Solr
                ...
                solr_result = solrh.upload_to_solr(flatten_resume_solr)
                print(f"Result from Solr: {solr_result}")

            except Exception as e:
                file_success = False
                error_message = f"Error storing flattened data for {file.filename}: {str(e)}"
                error_list.append(error_message)
                return file_success, error_list, flatten_resume
...
def construct_query(filters: FilterRequest) -> str:
    ...
    if filters.advanced_filters:
        for advanced_filter in filters.advanced_filters:
            ...
            weight = PREFERENCE_WEIGHTS.get(advanced_filter.preference, 0) * 10
